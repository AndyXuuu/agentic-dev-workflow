...
- Use `ax-sdd` only when the target repository's own `AGENTS.md` explicitly adopts Domain SDD and its existing manifest/index are authoritative. Absence of that project-local adoption is a hard routing boundary: do not initialize or synchronize SDD routing/artifacts there. A project may keep one explicit non-adoption sentence as a standalone guard. Domain SDD supports `ax-pipeline`; it does not replace delivery stages or provide a CLI.
- The Domain Index is the sole Domain navigation graph; select one primary Domain from operation/Owner evidence and register affected Domains only when their owned contract changes. Dictionary and Traceability are supporting lookup roots, not alternate Domain routing graphs. Dictionary Term overlap is candidate evidence, not ownership; tied candidates must be resolved before canonical edits. `default_domain` is for explicitly marked read-only initial navigation only.
- Manifest Artifact `references` form the direct dependency graph; every Artifact must be reachable from the Domain Index, Dictionary, or Traceability roots, while asset bytes are loaded only when the task or projection check touches them.
- Domain SDD contains current definitions, machine-readable contracts, declared assets and traceable Acceptance Boundaries—not Proposals, task lists, exploration, rejected alternatives, Agent reasoning, history or generated archives.
