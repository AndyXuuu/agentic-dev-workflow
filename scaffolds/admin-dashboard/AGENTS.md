...
在写功能需求前，先建立行为相关的术语/对象注册表：为参与者、实体/资源、命令/事件、
状态和外部概念记录稳定 ID、规范名称、定义与身份/生命周期/Owner 边界、关键关系、
中英文或技术别名及证据状态。同一身份、生命周期、Owner 和可观察契约的翻译或同义词只能
映射为一个概念；后续新增名词必须复用、显式登记不同边界，或清理依赖内容，不能让名称
自然分叉成两个对象。
