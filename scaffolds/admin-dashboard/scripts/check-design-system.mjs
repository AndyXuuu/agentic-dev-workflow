...
for (const sectionId of ['foundation-tokens', 'controls', 'components', 'list-patterns', 'charts', 'states']) {
...
for (const control of ['<input', '<select']) {
  if (!catalog.includes(control)) failures.push(`Design System Catalog 缺少真实 ${control.slice(1)} 控件示例`)
}
