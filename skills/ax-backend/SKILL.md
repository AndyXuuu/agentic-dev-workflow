---
name: ax-backend
description: 跨项目的后端实现与审查流程。用于非平凡的 API、RPC、服务、领域规则、权限、数据库、事务、缓存、队列、事件、外部集成、迁移或后端 Bug 改动；覆盖契约、数据所有权、一致性、并发、幂等、安全和验证。不用于仅需按 AGENTS.md Fast Path 处理的明确、局部、低风险小改动。
---
