...
不要在本仓库持久化项目绝对路径、已接入项目清单或同步状态。目标按本次任务即时确定：

1. 当前工作目录位于具体 Git 仓库时，默认只处理该仓库。
2. 用户明确给出仓库路径或项目列表时，只处理这些目标。
3. 用户要求查询、复核或同步多个适配器，但上下文无法唯一确定范围时，先询问仓库路径列表或一个边界明确的父目录；不要扫描整个用户目录，也不要猜测其他机器的路径。
4. 用户提供父目录后，只发现其中的 Git root 和仓库自有 `.agents/skills/`，排除 `.venv`、`node_modules`、vendor、缓存和第三方依赖。
5. 顶层规则变化需要同步时，在本次确认的目标集合内按领域筛选，逐个比较 `AGENTS.md`、适配 Skill、Owner 地图和验证入口；只报告本次实际核验结果，不持久化全局状态。
